...
function save()
{
	...
	if($f_id==0){
		$sql ="insert {pre}cj_filters (f_name,f_object,f_type,f_content,f_strstart,f_strend,f_rep,f_flag,f_pid,f_sys) values('".$f_name."','".$f_object."','".$f_type."','".$f_content."','".$f_strstart."','".$f_strend."','".$f_rep."','".$f_flag."','".$f_pid."','0') ";
	}
	else{
...
<table width="96%" border=0 align=center cellpadding="4" cellSpacing=0 class=tb >
	...
	<td colspan="2">
		...
		<input name="Cancel" class="btn" type="button" id="Cancel" value="取&nbsp;&nbsp;消" onClick="window.location.href='collect_vod_filters.php'">
	</td>
	</tr>
</table>
</form>
...
function main()
{
	...
	$sql="select a.*,b.p_name from {pre}cj_filters a left join {pre}cj_vod_projects b on a.f_pid=b.p_id where 1=1 and f_sys=0 ";
	if ($project!= "") {
...
$(document).ready(function(){
	$("#btnDel").click(function(){
		if(confirm('确定要删除吗')){
			$("#form1").attr("action","collect_vod_filters.php?action=del");
			$("#form1").submit();
		}
		else{return false}
	});
	$("#btnFlag1").click(function(){
		$("#form1").attr("action","collect_vod_filters.php?action=sflag&f_flag=1");
		$("#form1").submit();
	});
	$("#btnFlag2").click(function(){
		$("#form1").attr("action","collect_vod_filters.php?action=sflag&f_flag=0");
		$("#form1").submit();
	});
	$("#btnAdd").click(function(){
		location.href='collect_vod_filters.php?action=add';
	});
});
</script>
<table width="96%" border="0" align="center" cellpadding="3" cellspacing="1">
	...
	<option value="?{pre}cj_vod_projects=0" <?php if ($project=="0"){ echo "selected";} ?>>全局转换</option>
	<?php echo makeSelect("{pre}cj_vod_projects","p_id","p_name","","collect_vod_filters.php","&nbsp;|&nbsp;&nbsp;",$project); ?>
	</select>
	</td>
	</tr>
</table>
...
<table class=tb >
	...
	<td>
		</a>&nbsp;<a href="collect_vod_filters.php?action=edit&f_id=<?php echo $row["f_id"]?>">修改</a>
	</td>
    </tr>
<?php
		}
...
<tr>
	...
	<td colspan="8">
    <?php echo pagelist_manage($pagecount,$pagenum,$nums,app_pagenum,"collect_vod_filters.php?{pre}cj_vod_projects=".$project."&page={p}") ?>
	</td>
	</tr>
</table>
</form>
