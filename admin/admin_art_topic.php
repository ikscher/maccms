...
$_SESSION["upfolder"] = "../upload/arttopic";
switch($action)
{
	case "editall" : editall();break;
	default : headAdmin ("文章专题管理");main();break;
}
dispseObj();
...
function main()
{
	...
	$sql = "SELECT count(*) FROM {pre}art_topic";
	$nums = $db->getOne($sql);
	$pagecount=ceil($nums/app_pagenum);
	$sql = "SELECT * FROM {pre}art_topic ORDER BY t_sort,t_id ASC";
	$rs = $db->query($sql);
?>
...
$(document).ready(function(){
	...
	$("#btnDel").click(function(){
			if(confirm('确定要删除吗')){
				$("#form1").attr("action","admin_ajax.php?action=del&flag=batch&tab={pre}art_topic");
				$("#form1").submit();
			}
			else{return false}
	});
...
function edit(id)
{
	...
	$("#flag").val("edit");
	$('#win1').window('open');
	$('#form2').form('load','admin_ajax.php?action=getinfo&tab={pre}art_topic&col=t_id&val='+id);
}
</script>
<table class="tb">
<form action="" method="post" id="form1" name="form1">
    ...
    <tr align="center" >
	<td colspan="8">
		<?php echo pagelist_manage($pagecount,$pagenum,$nums,app_pagenum,"admin_art_topic.php?page={p}")?>
	</td>
    </tr>
</table>
</form>
...
<form action="admin_ajax.php?action=save&tab={pre}art_topic" method="post" name="form2" id="form2">
<table class="tb">
	...
	<tr>
     <td>专题图片：</td>
      <td><input id="t_pic" size=50 value="" name="t_pic" ><br>&nbsp;<iframe src="editor/uploadshow.php?action=art&id=t_pic" scrolling="no" topmargin="0" width="320" height="24" marginwidth="0" marginheight="0" frameborder="0" align="center"></iframe>
	  </td>
    </tr>
	<tr>
     <td>排序：</td>
