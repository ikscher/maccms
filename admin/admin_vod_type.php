...
function editall()
{
	...
	foreach($ids as $id){
		...
		$db->Update ("{pre}vod_type",array("t_name","t_enname", "t_sort","t_template","t_vodtemplate","t_playtemplate","t_downtemplate","t_key","t_des"),array($t_name,$t_enname,$t_sort,$t_template,$t_vodtemplate,$t_playtemplate,$t_downtemplate,$t_key,$t_des),"t_id=".$id);
	}
	updateCacheFile();
	echo "修改完毕";
}
...
function getTypeList($t_pid)
{
	...
	$sql = "SELECT t_id,t_name,t_enname,t_sort,t_pid,t_hide,t_template,t_vodtemplate,t_playtemplate,t_downtemplate,t_key,t_des FROM {pre}vod_type WHERE t_pid = ".$t_pid. " ORDER BY t_sort,t_id ASC";
	$rs = $db->query($sql);
	$tempnum = $tempnum + 3;
...
?>
	<tr>
		...
		<td><input size="12" type="text" name="t_playtemplate<?php echo $row["t_id"]?>" value="<?php echo $row["t_playtemplate"]?>"></td>
		<td><input size="12" type="text" name="t_downtemplate<?php echo $row["t_id"]?>" value="<?php echo $row["t_downtemplate"]?>"></td>
