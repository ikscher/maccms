...
function configsave()
{
	...
	$str.= "define(\"app_vodplayersort\"," . trim(be("post","app_vodplayersort"))  . ");        //视频播放器显示顺序0添加顺序，1全局顺序" . "\n";
	$str.= "define(\"app_vodmaccjsname\"," . trim(be("post","app_vodmaccjsname"))  . ");        //联盟资源库入库时重名判断条件0名称，1名称+分类" . "\n";
    ...
    $str.= "define(\"app_voddownviewtype\"," . trim(be("post","app_voddownviewtype"))  . ");        //视频播放页 浏览模式, 0=仿伪静态模式,1=asp动态模式,2=rewrite伪静态,3=静态每数据一页,4=静态每集一页,5=静态每组一页,6=静态全站一页" . "\n";
	...
	$str.= "define(\"app_weekpoint\","  . trim(be("post","app_weekpoint"))  . ");        //包周所需积分数" . "\n\n";	
	$str.= "define(\"app_monthpoint\","  . trim(be("post","app_monthpoint"))  . ");        //包月所需积分数" . "\n\n";	
	$str.= "define(\"app_yearpoint\","  . trim(be("post","app_yearpoint"))  . ");        //包年所需积分数" . "\n\n";	
...
<form method="POST" action="?action=configplaysave" id="form1" name="form1">
  <table class="tb">
    ...
    <tr>
      <td>预加载地址：</td>
      <td>
          <input name="loadads" type="text" id="loadads" value="<?php echo $loadads?>" size="50">
