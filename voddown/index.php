<?php
	...
	if (app_voddownviewtype==0 || app_voddownviewtype==2){
	...
	if (app_user==1){
		if (!getUserPopedom($row["d_type"],"down")){ showMsg ("您没有权限浏览此栏目下的内容,请联系管理员获取权限!","../user/"); }
		if ($row["d_stintdown"] > 0 && isN($userid)){ alertUrl ("此为收费数据请先登录再操作","../user/"); exit;}
		if (!isN($userid)){
			...
			if ($rowuser){
				...
				if ($rowuser["u_flag"] == 1){
					...
					if ($rowuser["u_points"] < $row["d_stintdown"]){
						if (strpos(",".$rowuser["u_downs"],",".$id.",") > 0){ $stat = true;}
						if (!$stat){ $str = "对不起,您的积分不够，无法观看收费数据，请推荐本站给您的好友、赚取更多积分";}
					}
					$userpoint = $rowuser["u_points"] - $row["d_stintdown"];
				}
				if (!isN($str)){ alertUrl ($str,"../user/");exit;}
				if (strpos(",".$rowuser["u_downs"],",".$id.",") > 0){ $stat = true;}
				if (!$stat){
					$playrecords = "," . $rowuser["u_downs"] . $id . ",";
					$playrecords = replaceStr($playrecords,",,",",");
					$db->Update ("{pre}user" ,array("u_points","u_downs"),array($userpoint,$playrecords),"u_id=".$userid);
				}
			}
			unset($rowuser);
		}
	...
	$template->loadvod ($row,$typearr,"down");
	unset($row);
	echo  $template->html;
	dispseObj();
?>